...
{
    public partial class QuizPage5ViewModel : ObservableObject
    {
        string uid = Preferences.Default.Get("SavedUserUid", "");

        private bool _locked;       // true after CheckAnswer is called; reset on NextQuestion
        ...
        {
            using var stream = await FileSystem.OpenAppPackageFileAsync("quiz5data.json");
        ...
        public async Task SaveQuizResults()
        {
            ...
            await QuizFunc.SaveQuizProgressAsync(storyNumber: 5, quizScore: Score);
        }
    }
}
